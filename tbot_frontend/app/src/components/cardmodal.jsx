...
const getEmojiIcon = (emoji) => {
  ...
  const iconMap = {
    brainz: { url: MANUAL_STAT_IMAGE_LINKS.cost, alt: "Brainz" },
    strength: { url: MANUAL_STAT_IMAGE_LINKS.strength, alt: "Strength" },
    health: { url: MANUAL_STAT_IMAGE_LINKS.health, alt: "Health" },
    sun: { url: MANUAL_STAT_IMAGE_LINKS.sun, alt: "Sun" },
    healthstrength: {
      url: MANUAL_STAT_IMAGE_LINKS.healthstrength,
      alt: "Health and Strength",
    },
    antihero: { url: MANUAL_STAT_IMAGE_LINKS.antihero, alt: "Anti-Hero" },
    strikethrough: {
      url: MANUAL_STAT_IMAGE_LINKS.strikethrough,
      alt: "Strikethrough",
    },
    deadly: { url: MANUAL_STAT_IMAGE_LINKS.deadly, alt: "Deadly" },
    special: { url: MANUAL_STAT_IMAGE_LINKS.special, alt: "Special" },
    freeze: { url: MANUAL_STAT_IMAGE_LINKS.freeze, alt: "Freeze" },
    bullseye: { url: MANUAL_STAT_IMAGE_LINKS.bullseye, alt: "Bullseye" },
    frenzy: { url: MANUAL_STAT_IMAGE_LINKS.frenzy, alt: "Frenzy" },
    armored: { url: MANUAL_STAT_IMAGE_LINKS.armored, alt: "Armored" },
    overshoot: { url: MANUAL_STAT_IMAGE_LINKS.overshoot, alt: "Overshoot" },
    untrickable: {
  ...
  const classImage = MANUAL_STAT_IMAGE_LINKS[parsed.key];
  if (classImage) {
    return { url: classImage, alt: parsed.name };
  }
  return null;
};
const getTitleClassIcon = (title) => {
  ...
  const className = normalizeEmojiName(match[1]);
  const classNames = [
    "guardian",
    "kabloom",
    "megagrow",
  ...
  return { url: MANUAL_STAT_IMAGE_LINKS[className], alt: match[1] };
};
    ...
    matches.forEach((match, index) => {
      ...
      if (match[1]) {
        ...
        if (icon) {
          ...
          parts.push(
            <span key={`ability-unknown-${index}`}> {emojiName} </span>,
          );
        }
      } else if (match[2]) {
  ...
  return (
    <div className="card-modal" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="modal-info">
        ...
        {displayTitle && (
          <div className="modal-card-title"> {displayTitle} </div>
        )}{" "}
